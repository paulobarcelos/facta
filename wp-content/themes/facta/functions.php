...
if ( function_exists( 'add_image_size' ) ) { 
	//add_image_size( 'gallery', 612, 878 ); //(cropped)
	add_image_size( 'magazine-cover', 612, 878 ); //(cropped)
	add_image_size( 'magazine-article', 612,612, true ); //(cropped)
}
...
function gallery_shortcode_carrousel($attr) {
	...
	if ($attachments) {
		...
		foreach ( $attachments as $attachment ) {
			...
			$output .= '<div class="item '.(($index == 0) ? 'active' : '').'">';
			$output .= '<a href="'.$image[0].'">';
			$output .= wp_get_attachment_image( $attachment->ID , 'full', false );
			$output .= '</a>';
			if($attachment->post_excerpt || $attachment->post_title){
